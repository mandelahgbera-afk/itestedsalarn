...
export default function AuthPage() {
  ...
  useEffect(() => {
    if (!isLoading && user) {
      // Clear the safety timeout the moment user resolves — prevents the
      // error toast from firing in the window between timer expiry and unmount.
      if (signInTimeoutRef.current) {
        clearTimeout(signInTimeoutRef.current);
        signInTimeoutRef.current = null;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // signIn() returned OK — keep spinner while auth context resolves.
      // 8s hard cap in case onAuthStateChange never fires (network failure after
      // a successful API call). Nulling the ref prevents a late-firing callback
      // from showing an error toast if the component unmounts just before the
      // timeout fires.
      if (signInTimeoutRef.current) {
        clearTimeout(signInTimeoutRef.current);
        signInTimeoutRef.current = null;
      ...
      signInTimeoutRef.current = setTimeout(() => {
        // Guard: ref is nulled by the redirect effect before firing if auth resolved
